...
@Slf4j
public class JwtTokenOwnerInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        //1、从请求头中获取令牌
        String token = request.getHeader(jwtProperties.getOwnerTokenName());

        //2、校验令牌
        try {
            log.info("jwt校验:{}", token);
            Claims claims = JwtUtil.parseJWT(jwtProperties.getOwnerSecretKey(), token);
